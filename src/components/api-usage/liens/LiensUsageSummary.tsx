...
export const LiensUsageSummary = ({
  ...
  return <Card>
      ...
      <CardContent>
        {isLoading ? <Skeleton className="h-8 w-24" /> : <div className="text-2xl font-bold">{propertyLiensRequests.toLocaleString()}</div>}
        <p className="text-xs text-muted-foreground mt-1">Total property lien searches today</p>
      </CardContent>
    </Card>;
};